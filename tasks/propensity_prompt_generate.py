...
class PromptBuilder:
    ...
    def build_user_prompt(
        ...
        ### 우리 가게 현황 요약
        * **주요 고객 분석:**
          - (그래프와 연령 믹스 데이터를 해석하여 어떤 연령대의 고객이, 왜 많이 오는지 설명)
          - (가장 비중이 높은 고객층과 낮은 고객층을 비교하여 특징을 설명)
        * **고객 행동 패턴:**
          - (오디언스 데이터와 대표 규칙을 바탕으로 신규/재방문/직장인 등 고객들의 특징적인 행동을 요약)
        * **강점 및 기회:**
        ...
        * **핵심 메시지:**
          - (고객의 마음을 사로잡을 첫 번째 핵심 메시지)
          - (가게의 장점을 어필할 두 번째 핵심 메시지)
        * **기대 효과 및 목표 KPI:**
          - (이 전략을 실행했을 때 예상되는 긍정적인 변화를 설명)
          - **목표 KPI:** (예: 예약 취소율 30% 감소, 재방문 고객 15% 증가 등 구체적인 수치 제시)
        * **주요 리스크 및 대응 방안:**
          - **리스크:** (높은 예약 취소율 등 데이터에서 발견되는 잠재적 위험 요소를 지적하고, 이것이 의미할 수 있는 운영상의 문제를 설명)
          - **대응 방안:** (리스크를 관리하고 고객 경험을 개선하기 위한 구체적인 액션 아이템 제안)

        ### 추천 마케팅 채널 및 키워드
        * **추천 채널:**
          - **1. (채널명):** (이 채널을 추천하는 이유와 활용 방안을 1~2줄로 설명)
          - **2. (채널명):** (설명)
          - **3. (채널명):** (설명)
        * **추천 키워드:**
          - (추천 키워드 Raw Data에서 가장 효과적인 키워드 5개를 쉼표로 구분하여 목록으로 제시)
        * **확장을 위한 대안 타겟:**
          - **대안 타겟 1:**
            - **각색 포인트:** (해당 타겟을 공략하기 위해 기존 전략을 어떻게 변형해야 할지 설명.)
          - **대안 타겟 2:**
            - **각색 포인트:** (해당 타겟을 공략하기 위해 기존 전략을 어떻게 변형해야 할지 설명.)
