...
아래 타겟 브리프에 맞춰 ‘한글’로 톤·메시지·채널/키워드를 제안하세요.
...
[작성 원칙(요약)]
- 쉬운 말·불릿 위주, 한 문단 3줄 이내.
- 숫자→뜻→액션(당장 할 일 1~2개).
- ‘높음/보통/낮음’·%로 비교, 과한 단정 금지.

[용어(짧게)]
- Age Mix: 연령 비중(0~1=0~100%).
- NEW 신규, REU 재구매, RES 예약, WORK 직장인, FLOW 유동.
- cancel_high: 취소율 신호, delivery_high: 배달 비중 신호.

[그래프 해석 팁]
- PNG 2장: (1) Age Mix, (2) Audience.
- 막대가 클수록 큼(0~1을 %로 말하기), 최고/최저 먼저 + 시사점 1문장.

[출력]
1) 한 줄 콘셉트
2) 핵심 메시지×3(각 20자 내)
3) 그래프 요약(숫자→뜻→액션, cancel/delivery 포함)
4) 채널 가이드(Top N, KPI/포인트 1~2줄)
5) 키워드 10개
6) KPI 가설 + A/B 2안
7) 대안 타깃 2 & 각색 포인트
8) 리스크·대응(오해 방지 문구)
